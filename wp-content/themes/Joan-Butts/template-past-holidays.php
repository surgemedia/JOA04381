			...
			<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
				<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
					...
					<section class="entry-content clearfix" itemprop="articleBody">
						<?php the_content(); ?>
						<?php 
							if(get_field('past_holiday')): ?>
 								<ul id="holiday-list">
									<?php while(has_sub_field('past_holiday')): ?>
									 	<li>
											<img class="holiday-img" src="<?php the_sub_field('past_holiday_image'); ?>">
											<h2 class="holiday-title"><?php the_sub_field('past_holiday_name'); ?></h2>
											<p><?php the_sub_field('past_holiday_description'); ?></p>
											<a class="btn btn-primary" href="/past-holidays/gallery/">View Gallery</a>
										</li>
									<?php endwhile; ?>
								</ul>
							<?php endif; ?>
