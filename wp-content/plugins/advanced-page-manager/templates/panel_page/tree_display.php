 ...
 *  2:published, 3:private, 4:trash, 5:scheduled, -2:in APM tree but not in WP anymore
 */
							...
							<strong>
								<?php if( $node->status > -2 ) : ?>
									<?php if( $node->status == 4 ) : ?>
										<span class="node-trashed"><?php echo '['. __('Trash', ApmConfig::i18n_domain) .'] '. esc_html( $node->title )?></span>
									<?php else: ?>
										<a class="row-title" href="<?php echo get_bloginfo('wpurl').'/wp-admin/post.php?post='.$node->wp_id.'&action=edit' ?>" title="<?php echo esc_attr( $node->title ) ?>"><?php echo esc_html( $node->title ) ?></a>
									<?php endif ?>
								<?php else: ?>
									<span class="node-deleted-from-wp"><?php echo esc_html( $node->title ) ?></span>
								<?php endif?>
							</strong>

							<div class="wrap-edit-title field-rename">
								<input type="text" value="<?php echo esc_attr( $node->title ) ?>" />
